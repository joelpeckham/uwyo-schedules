...
| Course / section description (HTML) | GET | `/searchResults/getCourseDescription` | Query: `term=<termCode>`, `courseReferenceNumber=<crn>` | Same as `getTerms` | **HTML** fragment: catalog course description plus optional **Section information text** (exam reservations, lab notes). Not included in `searchResults` rows; fetch per CRN when needed. |
